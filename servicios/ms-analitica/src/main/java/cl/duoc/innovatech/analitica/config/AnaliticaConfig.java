package cl.duoc.innovatech.analitica.config;
...
public class AnaliticaConfig {
    ...
    // Un solo Customizer configura ambos breakers (proyectos y recursos)
    ...
    public Customizer<Resilience4JCircuitBreakerFactory> breakersConfig() {
        ...
        var tl = TimeLimiterConfig.custom()
                ...
                .build();
        return factory -> {
            factory.configure(b -> b.circuitBreakerConfig(cb).timeLimiterConfig(tl).build(), "proyectos");
            factory.configure(b -> b.circuitBreakerConfig(cb).timeLimiterConfig(tl).build(), "recursos");
        };
    }
}
