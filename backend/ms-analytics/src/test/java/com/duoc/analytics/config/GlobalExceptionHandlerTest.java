...
class GlobalExceptionHandlerTest {
    ...
    private final MockHttpServletRequest req = new MockHttpServletRequest("GET", "/analytics/kpis");

    @Test
    void illegalArgument_is400() {
        var r = handler.handleBadRequest(new IllegalArgumentException("bad arg"), req);
        assertThat(r.getStatusCode().value()).isEqualTo(400);
        assertThat(r.getBody().path()).isEqualTo("/analytics/kpis");
    }

    @Test
    void responseStatus_passesThrough() {
        var r = handler.handleStatus(new ResponseStatusException(HttpStatus.BAD_REQUEST, "rango"), req);
        assertThat(r.getStatusCode().value()).isEqualTo(400);
    }
